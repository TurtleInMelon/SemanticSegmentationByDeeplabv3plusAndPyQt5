...
def _process_batch(sess, original_images, semantic_predictions, image_names,
  ...
  # print(original_images.shape)    (1, 1024, 2048, 3)
  # print(semantic_predictions.shape)   (1, 1024, 2048)
  num_image = semantic_predictions.shape[0] # 1 图片数量
  for i in range(num_image):
    image_height = np.squeeze(image_heights[i]) # 1024
    image_width = np.squeeze(image_widths[i])   # 2048
    original_image = np.squeeze(original_images[i]) # (1024, 2048, 3)
    semantic_prediction = np.squeeze(semantic_predictions[i])   # (1024, 2048)
    crop_semantic_prediction = semantic_prediction[:image_height, :image_width]  # (1024, 2048)

    # Save image.    保存图片
    save_annotation.save_annotation(
        original_image, save_dir, _IMAGE_FORMAT % (image_id_offset + i),    # _IMAGE_FORMAT = '%06d_image'
        add_colormap=False)

    # Save prediction.  保存预测结果
    save_annotation.save_annotation(
...
def main(unused_argv):
  ...
  dataset = data_generator.Dataset(     # 获取相应数据集
      ...
      crop_size=[int(sz) for sz in FLAGS.vis_crop_size],     #crop_size 默认为513,513
      ...
      model_variant=FLAGS.model_variant,     # 模型的变体　默认为mobilenet_v2  本次训练为 xception_65
  ...
  save_dir = os.path.join(FLAGS.vis_logdir, _SEMANTIC_PREDICTION_SAVE_FOLDER)   # 创建存放文件夹
      ...
      with tf.train.MonitoredSession(
        ...
        while not sess.should_stop():
          ...
          _process_batch(sess=sess,
                         original_images=samples[common.ORIGINAL_IMAGE],    #  <tf.Tensor 'IteratorGetNext:4' shape=(?, ?, ?, 3) dtype=uint8>
                         semantic_predictions=predictions,  # <tf.Tensor 'ArgMax:0' shape=(1, 1024, 2048) dtype=int64>
                         image_names=samples[common.IMAGE_NAME],     # <tf.Tensor 'IteratorGetNext:2' shape=(?,) dtype=string>
                         image_heights=samples[common.HEIGHT],  #  <tf.Tensor 'IteratorGetNext:0' shape=(?,) dtype=int64>
                         image_widths=samples[common.WIDTH],     # <tf.Tensor 'IteratorGetNext:5' shape=(?,) dtype=int64>
                         image_id_offset=image_id_offset,   # 0
                         save_dir=save_dir,  # 语义分割结果放置的路径
                         raw_save_dir=raw_save_dir,
                         train_id_to_eval_id=train_id_to_eval_id)   # 只有cityscape中不为None
