...
def save_annotation(label,  # 生成分割图并保存
                    save_dir,
                    filename,
                    add_colormap=True,
  ...
  if add_colormap:  # 默认为True
    # print(label.shape)    (1024, 2048)
    colored_label = get_dataset_colormap.label_to_color_image(   # 颜色标签 默认为（512， 3）
        label, colormap_type)
  else:
    colored_label = label
  ...
  pil_image = img.fromarray(colored_label.astype(dtype=np.uint8)) # 生成图像并保存
