...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,     #   <tf.Tensor 'logits/semantic/BiasAdd:0' shape=(?, 129, 129, 19) dtype=float32>
                                                  labels,               # 'label': <tf.Tensor 'label:0' shape=(?, 513, 513, 1) dtype=int32>
                                                  num_classes,          # 19
                                                  ignore_label,         # 255
                                                  loss_weight=1.0,
  ...
  """Adds softmax cross entropy loss for logits of each scale.
    add_softmax_cross_entropy_loss_for_each_scale(  # softmax交叉熵损失函数
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    weights = utils.get_label_weight_mask(  # 各个标签权重
    ...
    if gt_is_matting_map:   # 默认为False
      ...
      train_labels = tf.one_hot(    # 转成one_hot向量
    ...
    with tf.name_scope(loss_scope, default_loss_scope,
      ...
      pixel_losses = tf.nn.softmax_cross_entropy_with_logits_v2(
          ...
          logits=logits,
          name='pixel_losses')
      weighted_pixel_losses = tf.multiply(pixel_losses, weights)    # 乘上权重
...
def get_model_init_fn(train_logdir,     # 从ckpt文件中获取模型初始化权重
  ...
  exclude_list = ['global_step']    # 不从ckpt中加载的变量     'logit' 'decoder'等
