...
@slim.add_arg_scope
def xception_module(inputs,     # 构建每个xception模块，每个模块由2个深度可分离卷积和一个可选的残差块类型决定
                    depth_list,
                    skip_connection_type,   # 决定残差块的类型  在middle flow中使用sum 在Exit flow中最后一个block用None
                    stride,
                    kernel_size=3,
                    unit_rate_list=None,
                    rate=1,
  ...
  """
  if len(depth_list) != 3:  # 每个Xception模块由3个子深度可分离卷积组成，所以depth_list长度为3
  ...
  with tf.variable_scope(scope, 'xception_module', [inputs]) as sc:
    residual = inputs   # 保存input值，因为每个模块需要用两次input

    def _separable_conv(features, depth, kernel_size, depth_multiplier, # 深度可分离卷积，同时保持原深度
                        regularize_depthwise, rate, stride, scope):
      """Separable conv block."""
      if activation_fn_in_separable_conv:
        activation_fn = tf.nn.relu6 if use_bounded_activation else tf.nn.relu
      else:
        if use_bounded_activation:  # 边界激活函数
    ...
    for i in range(3):
      ...
      # 每个模块中3个子深度可分离卷积进行相连，第三个子模块的最大池化改为步幅stride为2的深度可分离卷积
    ...
    if skip_connection_type == 'conv':  # 残差块使用的网络连接类型为卷积
      shortcut = slim.conv2d(inputs,    # 快捷连接  深度为第三个子模块的深度，stride为2,1x1的卷积
                             depth_list[-1],
                             [1, 1],
                             stride=stride,
                             activation_fn=None,
      ...
      outputs = residual + shortcut # 进行合并
      if use_bounded_activation:
      ...
      outputs = residual + inputs   # xception中间流使用
      if use_bounded_activation:
        outputs = tf.nn.relu6(outputs)
    elif skip_connection_type == 'none':    # 不适用残差块
      outputs = residual
    else:
...
@slim.add_arg_scope
def stack_blocks_dense(net,     # 将每个block进行级联
                       blocks,
                       output_stride=None,
                       outputs_collections=None):
  ...
  for block in blocks:  # 获取每一个Xception块
...
def xception(inputs,        # Xception网络的入口
             blocks,
             num_classes=None,
             is_training=True,
             global_pool=True,
    ...
    with slim.arg_scope([slim.conv2d,
      ...
      with slim.arg_scope([batch_norm], is_training=is_training):
        ...
        if output_stride is not None:
          ...
          output_stride //= 2
        # Root block function operated on inputs.   开始卷积 网络路口
...
def xception_block(scope,    # 一个block为3个深度可分离卷积级联外加一个可选类型的残差块
                   depth_list,
  ...
  return Block(scope, xception_module, [{
      ...
      'use_squeeze_excite': use_squeeze_excite,
      'se_pool_size': se_pool_size,
  }] * num_units)   # num_units 为 每个xception_block重复的次数，除了middle flow,其余均为1
