...
networks_to_feature_maps = {
    ...
    'xception_65': {
        DECODER_END_POINTS: {
            2: ['entry_flow/block1/unit_1/xception_module/'     # 改进decoder模型必须增加，使得decoder模块为16，8，4，2，更加充分利用低级特征与高级特征进行融合
                'separable_conv2_pointwise'],
            4: ['entry_flow/block2/unit_1/xception_module/'
                'separable_conv2_pointwise'],
            8: ['entry_flow/block3/unit_1/xception_module/'
...
def mean_pixel(model_variant=None): # 平均像素
...
def extract_features(images,        # 调用相应的主干网络，提取特征
                     output_stride=8,
                     multi_grid=None,
  ...
  if 'resnet' in model_variant:
    ...
    arg_scope = arg_scopes_map[model_variant](  # 获取参数 网络的arg_scope
        weight_decay=weight_decay,
    ...
    features, end_points = get_network(     # 获取Xception网络,并提取网络特征
        model_variant, preprocess_images, preprocessed_images_dtype, arg_scope)(
            inputs=images,
            num_classes=num_classes,
            is_training=(is_training and fine_tune_batch_norm),
...
def get_network(network_name, preprocess_images,    # 获取相应的主干网络
  ...
  arg_scope = arg_scope or arg_scopes_map[network_name]()   # 获取arg_scope
  def _identity_function(inputs, dtype=preprocessed_images_dtype):
    return tf.cast(inputs, dtype=dtype)
  if preprocess_images:     # 若需要预处理，将像素值映射到[-1,1]
    preprocess_function = _PREPROCESS_FN[network_name]
  else:
    preprocess_function = _identity_function
  func = networks_map[network_name]     # 获取相应的网络
  @functools.wraps(func)        # 若无此修饰器，调用func时，无法调用到net模型的函数
