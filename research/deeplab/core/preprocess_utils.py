...
def flip_dim(tensor_list, prob=0.5, dim=1):  # 随机翻转输入tensor的一个维度
...
def pad_to_bounding_box(image, offset_height, offset_width, target_height,
  ...
  Args:
    ...
    pad_value: Value to pad the image tensor with. 填充值
  ...
  with tf.name_scope(None, 'pad_to_bounding_box', [image]):
    image = tf.convert_to_tensor(image, name='image')   # 转成tensor
...
def _crop(image, offset_height, offset_width, crop_height, crop_width): # 裁剪图像
  ...
  """
  original_shape = tf.shape(image)  # 原始图像大小

  if len(image.get_shape().as_list()) != 3:
    raise ValueError('input must have rank of 3')
  original_channels = image.get_shape().as_list()[2]    # 原始图像通道维
...
def random_crop(image_list, crop_height, crop_width):   # 随机裁剪图片
  ...
  offset_height = tf.random_uniform(    # 随机裁剪高度位置
      [], maxval=max_offset_height, dtype=tf.int32)
  offset_width = tf.random_uniform( # 随机裁剪宽度位置
...
def get_random_scale(min_scale_factor, max_scale_factor, step_size):    # 获取随机一个变换尺度
  ...
  if min_scale_factor == max_scale_factor:  # 若相等，直接返回
    return tf.cast(min_scale_factor, tf.float32)

  # When step_size = 0, we sample the value uniformly from [min, max).
  # 当步长为0时，从最大最小scale中=之间随机选取
  if step_size == 0:
  ...
  '''
      lin_space(
          ...
          均分计算指令,在间隔中生成值. 
          从一开始就生成 num 平均间隔值的序列.如果 num > 1,则序列中的值通过 stop - start / num - 1 增加,
          因此最后一个是完全停止的.
    '''
  scale_factors = tf.lin_space(min_scale_factor, max_scale_factor, num_steps)
  # 将要变换的尺度列表 随机打乱
  shuffled_scale_factors = tf.random_shuffle(scale_factors)
  return shuffled_scale_factors[0]  #返回第一个元素


def randomly_scale_image_and_label(image, label=None, scale=1.0):   # 通过随机获得的尺度进行image和label的变化
  ...
  if scale == 1.0:  # scale为1 直接返回原尺度
    return image, label
  image_shape = tf.shape(image)
  new_dim = tf.cast(    # 新的图像尺度
      tf.cast([image_shape[0], image_shape[1]], tf.float32) * scale,
      tf.int32)

  # Need squeeze and expand_dims because image interpolation takes
  # 4D tensors as input.
  image = tf.squeeze(tf.image.resize_bilinear(   # 双线性插值
      tf.expand_dims(image, 0),     # 先扩张维度  图像插值需要4Dtensor
      new_dim,
      align_corners=True), [0])
  if label is not None:
    label = tf.image.resize(    # 标签进行缩放
        label,
        new_dim,
...
def resolve_shape(tensor, rank=None, scope=None):# 第二次上采样用到
