...
def predict_labels_multi_scale(images,      # 对尺度预测
                               model_options,
                               eval_scales=(1.0,),
  ...
  for i, image_scale in enumerate(eval_scales):
    ...
    if add_flipped_images:  # 默认不加左右翻转进行评估
...
def predict_labels(images, model_options, image_pyramid=None):  # 预测某一图片每个像素相应标签
  """Predicts segmentation labels.
  ...
  Returns:
    A dictionary with keys specifying the output_type (e.g., semantic
      ...
      over channels). Each prediction has size [batch, height, width].
  """
  outputs_to_scales_to_logits = multi_scale_logits( # 输入模型获取相应标签
      images,
      model_options=model_options,
  ...
  for output in sorted(outputs_to_scales_to_logits):
    ...
    if model_options.prediction_with_upsampled_logits:  # 双线性上采样
...
def multi_scale_logits(images,
                       model_options,
                       image_pyramid,
                       weight_decay=0.0001,     # 权重衰减， 使用 ResNet 0.0001   使用MobileNet-V2或者Xception中  weight_decay= 0.00004
                       is_training=False,
                       fine_tune_batch_norm=False,
  ...
  if not image_pyramid:     # 默认为None 只进行缩放尺度为1的原尺度输入
    image_pyramid = [1.0]
  crop_height = (    # 裁剪输入图像的高度 513  未指定是默认整个图像的高度
      model_options.crop_size[0]
      if model_options.crop_size else tf.shape(images)[1])
  crop_width = (    # 裁剪输入图像的宽度 513  未指定是默认整个图像的宽度
      model_options.crop_size[1]
      if model_options.crop_size else tf.shape(images)[2])
  if model_options.image_pooling_crop_size:     # 在image_pooling中返回的特征图大小  若为None， 则保持原尺寸
  ...
  # decoder中输出步长 若不为空  xception中为 4
  ...
  for image_scale in image_pyramid: # 默认为None 即为[ 1.0 ]
    if image_scale != 1.0:
      # 根据金字塔的尺度修改高度，宽度，crop_size，image大小
      ...
      # 根据图像金字塔的尺度相应的更改image_pooling的输出
  ...
  # Merge the logits from all the multi-scale inputs. 多尺度的输入 得到的特征图进行合并
  for output in sorted(model_options.outputs_to_num_classes):
...
def extract_features(images,    # 提取经过主干网络和ASPP后的特征 再最后经过1x1卷积之后加上一层dropout层
  ...
  features, end_points = feature_extractor.extract_features(    # 经过主干网络得到的特征图
      ...
      depth_multiplier=model_options.depth_multiplier,   # 深度乘子 默认为1.0  mobilenet中使用
      divisible_by=model_options.divisible_by,   # mobilenet中使用 默认为None
      weight_decay=weight_decay,    # 权重衰退 0.0004
      reuse=reuse,
      is_training=is_training,
      preprocessed_images_dtype=model_options.preprocessed_images_dtype,    # 预处理图像类型
      fine_tune_batch_norm=fine_tune_batch_norm,    # 微调BN层
      nas_architecture_options=model_options.nas_architecture_options,
      nas_training_hyper_parameters=nas_training_hyper_parameters,
      use_bounded_activation=model_options.use_bounded_activation)  # 使用边界激活函数 False
        ...
        with slim.arg_scope([batch_norm], **batch_norm_params):
          ...
          if model_options.add_image_level_feature:  # 添加图像水平特征 默认为True
            ...
            image_feature = slim.conv2d(
                image_feature, depth, 1,    # image_pooling出来的特征进行1x1卷积
          ...
          if model_options.atrous_rates:     # 空洞卷积
            ...
            for i, rate in enumerate(model_options.atrous_rates, 1):# rate 为空洞率
              ...
              branch_logits.append(aspp_features)   # 将空洞卷积提取的特征加入列表

          # Merge branch logits.
          concat_logits = tf.concat(branch_logits, 3)   # 将这些特征图进行按照通道的那个维度进行级联组合
...
def _get_logits(images,  # 获取每一个图像尺寸输入模型最终得到的特征图
                model_options,
                weight_decay=0.0001,
                reuse=None,
                is_training=False,
  ...
  """
  features, end_points = extract_features(  # 提取经过主干网络和ASPP后的特征
      images,   # image 经过缩放，左右翻转后得到的images
      model_options,
      weight_decay=weight_decay,
      reuse=reuse,
      is_training=is_training,
      ...
      nas_training_hyper_parameters=nas_training_hyper_parameters)
  # decoder 模块
  if model_options.decoder_output_stride:   # xception resNet 参数输入为4   在mobilenetv2中为None
    crop_size = model_options.crop_size
    if crop_size is None:   # 若无指定裁剪大小 默认为原图大小
      crop_size = [tf.shape(images)[1], tf.shape(images)[2]]
    features = refine_by_decoder(   # decode后获取的特征图
        features,
        end_points,
        crop_size=crop_size,
  ...
  outputs_to_logits = {}
  for output in sorted(model_options.outputs_to_num_classes):
    if model_options.decoder_output_is_logits:  # 默认为False
...
def refine_by_decoder(features,     # decoder 提取特征
                      end_points,
                      crop_size=None,
  ...
  batch_norm_params = utils.get_batch_norm_params(  # BN层参数
      decay=0.9997,
      epsilon=1e-5,
      scale=True,
      is_training=(is_training and fine_tune_batch_norm),
      sync_batch_norm_method=sync_batch_norm_method)
  batch_norm = utils.get_batch_norm_fn(sync_batch_norm_method)  # BN层
  decoder_depth = decoder_filters    # 过滤器的数量 256
  projected_filters = 48    # 低级特征提取 1x1卷积使用的过滤器数 48 论文中为48
  if decoder_use_sum_merge: # 默认为False
    # When using sum merge, the projected filters must be equal to decoder
    # filters.
    projected_filters = decoder_filters
  if decoder_output_is_logits:  # 默认为False
    ...
    activation_fn = tf.nn.relu6 if use_bounded_activation else tf.nn.relu # 激活函数
    normalizer_fn = batch_norm
    conv2d_kernel = 3
  with slim.arg_scope(
      [slim.conv2d, slim.separable_conv2d],
    ...
    with slim.arg_scope([batch_norm], **batch_norm_params):
      with tf.variable_scope(DECODER_SCOPE, DECODER_SCOPE, [features]):
        ...
        decoder_stage = 0
        scope_suffix = ''
        for output_stride in decoder_output_stride:     # xcepion_64中为 4
          ...
          for i, name in enumerate(feature_list):
            ...
            decoder_features_list.append(
                slim.conv2d(    # 利用1x1提取特征 48filters     low_level features提取
            ...
            # Resize to decoder_height/decoder_width.上采样
            ...
            if decoder_use_sum_merge:   # 默认为False
              ...
              if not decoder_use_separable_conv:    # decoder用深度卷积 默认为True
                scope_suffix = str(i) + scope_suffix
              decoder_features = _decoder_with_concat_merge(    # decoder模块中的级联部分
                  decoder_features_list,
                  decoder_depth,
                  decoder_use_separable_conv=decoder_use_separable_conv,
                  weight_decay=weight_decay,
...
def _decoder_with_concat_merge(decoder_features_list,   # decoder中级联部分
                               decoder_depth,
                               decoder_use_separable_conv=True,
                               weight_decay=0.0001,
                               scope_suffix=''):
  ...
  """
  if decoder_use_separable_conv:    # 使用深度可分离卷积
    decoder_features = split_separable_conv2d(
        tf.concat(decoder_features_list, 3),    #级联在深度可分离卷积
        filters=decoder_depth,
        rate=1,
        weight_decay=weight_decay,
